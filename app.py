...
def build_taxonomy_index(taxonomy):
    """
    Build a set of meaningful words from the taxonomy generated by Phase 0.
    Universal — works for any industry because taxonomy is Claude-generated
    from the business description, not hardcoded.

    Returns a set of lowercase words that signal business relevance.
    A keyword containing ANY of these words is auto-relevant.

    Example for HVAC: {'furnace','boiler','ac','heating','cooling','drain',...}
    Example for dental: {'dental','tooth','teeth','implant','crown','cavity',...}
    """
    STOP = {'and','or','the','for','of','in','a','an','to','is','are','be',
            'with','as','at','by','from','on','how','what','why','when','which',
            'cost','guide','tips','page','post',
            'new','vs','versus','near','local','best','top','your','our'}
    index = set()
    for theme, subs in taxonomy.items():
        # Add meaningful words from theme name
        for word in re.split(r'[\s/\-_,]+', theme.lower()):
            if word and len(word) >= 2 and word not in STOP:
                index.add(word)
        # Add meaningful words from each sub-theme
        for sub in subs:
            for word in re.split(r'[\s/\-_,]+', sub.lower()):
                if word and len(word) >= 2 and word not in STOP:
                    index.add(word)
    return index

def is_relevant_by_taxonomy(kw, taxonomy_index):
    """
    Check if a keyword is relevant to the business using the taxonomy index.
    Universal — uses Claude-generated taxonomy, not hardcoded rules.

    Returns True if the keyword contains any taxonomy word → auto-relevant.
    Returns False if no match → needs Claude to decide.
    """
    if not taxonomy_index:
        return False   # No taxonomy → can't auto-classify, send to Claude
    kl = kw.lower()
    return any(word in kl for word in taxonomy_index)

def phase4_relevance_blogs(mapped, url_df, api_key, biz_desc, excl_str,
                           status_text, progress_bar, taxonomy=None):
    """
    Phase 4: Business relevance check — universal, no hardcoded rules.

    Pre-filter uses the Claude-generated taxonomy from Phase 0:
      - Keywords containing any taxonomy word → auto-RELEVANT (no Claude call)
      - Keywords with no taxonomy match → Claude decides

    This is universal: works for HVAC, dental, legal, SaaS — any business.
    The taxonomy is specific to this business, generated fresh each run.
    """
    progress_bar.progress(82)
    status_text.text("Phase 4: Checking business relevance...")

    # Build relevance index from Phase 0 taxonomy (universal — any industry)
    taxonomy_index = build_taxonomy_index(taxonomy) if taxonomy else set()

    # Keywords with a Phase 2 Claude match are confirmed relevant
    confirmed = {r['Keyword'].lower(): 'RELEVANT'
                 for r in mapped if r['Landing Page'] and r['Final Score'] > 0}

    # Split unmatched keywords into auto-relevant and needs-Claude
    auto_relevant = {}
    needs_claude  = []
    for r in mapped:
        if r['Landing Page']:
            continue   # Already confirmed above
        kw = r['Keyword']
        if is_relevant_by_taxonomy(kw, taxonomy_index):
            # Keyword matches taxonomy → auto-relevant, no Claude needed
            auto_relevant[kw.lower()] = 'RELEVANT'
        else:
            # No taxonomy match → Claude decides relevance
            needs_claude.append(kw)

    n_auto = len(auto_relevant)
    n_cl   = len(needs_claude)
    status_text.text(f"Phase 4: {n_auto:,} auto-relevant via taxonomy, "
                     f"sending {n_cl:,} to Claude...")

    rel_map = {}
    if needs_claude:
        rel_map = claude_relevance(api_key, needs_claude, biz_desc, excl_str,
                                   status_text, progress_bar, 82, 92)

    all_rel = {**confirmed, **auto_relevant, **rel_map}
    ...
    try:
        ...
        rel_map, mapped = phase_run("Phase 4 (Relevance)",
            lambda: phase4_relevance_blogs(mapped, url_df, api_key, biz_desc, excl_str,
                                           status_text, progress_bar, taxonomy))
